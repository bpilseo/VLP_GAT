...
from ..graph_attention.model import GAT
...
class BertImgModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings(config) # vocab_size = 30,522 | 768 imbedding vector
        self.encoder = CaptionBertEncoder(config)
        self.pooler = BertPooler(config)

        self.img_dim = config.img_feature_dim # 2054
        logger.info('BertImgModel Image Dimension: {}'.format(self.img_dim))
        self.img_feature_type = config.img_feature_type
        if hasattr(config, 'use_img_layernorm'): # check if 'use_img_layernorm' in config
        ...
        if config.img_feature_type == 'dis_code':
            ...
            self.code_embeddings = nn.Embedding(config.code_voc, config.code_dim, padding_idx=0)
            self.img_embedding = nn.Linear(config.code_dim, self.config.hidden_size, bias=True)
        else: # config.img_feature_type = 'frcnn'
            self.img_embedding = nn.Linear(self.img_dim, self.config.hidden_size, bias=True) # nn.Linear(2054, 768, bias=True)
            self.dropout = nn.Dropout(config.hidden_dropout_prob)
            if self.use_img_layernorm:
                self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.img_layer_norm_eps)

        self.apply(self.init_weights) # initialize weights in BertImgModel
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        embedding_output = self.embeddings(input_ids, position_ids=position_ids,
                token_type_ids=token_type_ids) # [B*2, max_seq_length (70), 768]
        ...
        if img_feats is not None:
            if self.img_feature_type == 'dis_code':
                ...
                img_embedding_output = self.img_embedding(img_feats) # nn.Linear(2054, 768, bias=True)
                if self.use_img_layernorm:
                    img_embedding_output = self.LayerNorm(img_embedding_output)

                # add dropout on image embedding
                img_embedding_output = self.dropout(img_embedding_output) # [B*2, max_img_seq_length(50), 768]

            # concatenate two embeddings
            embedding_output = torch.cat((embedding_output, img_embedding_output), 1) #[B*2, 120, 768]

            batch_num = embedding_output.shape[0]
            node_num = embedding_output.shape[1]

            gat_model = GAT(in_feat_dim=embedding_output.shape[2],
                                nhid=8,
                                out_feat_dim=embedding_output.shape[2],
                                dropout=0.6,
                                alpha=0.2, # leakyrelu
                                nheads=8,
                                batch_num = batch_num,
            ...
            embedding_output = gat_model(embedding_output)       

        encoder_outputs = self.encoder(embedding_output,            # self.encoder : CaptionBertEncoder  # [B*2, 120, 768]
                extended_attention_mask, head_mask=head_mask,
                encoder_history_states=encoder_history_states)      
        sequence_output = encoder_outputs[0] 
        pooled_output = self.pooler(sequence_output)                # self.pooler : BertPooler            # [B*2, 768]

        # add hidden_states and attentions if they are here
        outputs = (sequence_output, pooled_output,) + encoder_outputs[1:] #
        return outputs # outputs[0].shape = [B*2, 120, 768], outputs[1].shape = [B*2, 768]
...
class ImageBertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        if config.img_feature_dim > 0: # 2054
        ...
        if hasattr(config, 'classifier'):
            ...
            self.classifier = nn.Linear(config.hidden_size, self.config.num_labels)  # original | nn.Linear(768, 2)
        self.apply(self.init_weights) # initialize weights in ImageBertForSequenceClassification
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, 
            position_ids=None, head_mask=None, img_feats=None):
        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids, # self.bert = BertImgModel
                            attention_mask=attention_mask, head_mask=head_mask, img_feats=img_feats)
        pooled_output = outputs[1] # [B*2, 768]

        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output) # self.classifier: nn.Linear(768, 2)  ||   classification

        outputs = (logits,) + outputs[2:]  # add hidden states and attention if they are here # outputs[0].shape [B*2, 2]
        if labels is not None:
            if self.num_labels == 1: #  doing regression
                ...
                else: # cross_entropy [GQA, Retrieval, Captioning] # sfmx
                    loss_fct = CrossEntropyLoss()
                    loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            outputs = (loss,) + outputs
        return outputs  # outputs[0] : loss,   outputs[1] : [B*2, 2] (nn.Linear 통과한)
